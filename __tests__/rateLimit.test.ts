...
describe('rateLimit', () => {
  it('allows requests under the limit', async () => {
    ...
    expect(await rl(key, 3, 1000)).toBe(true)
    expect(await rl(key, 3, 1000)).toBe(true)
    expect(await rl(key, 3, 1000)).toBe(true)
  })
  ...
  it('blocks the request that exceeds the limit', async () => {
    ...
    await rl(key, 2, 1000)
    await rl(key, 2, 1000)
    expect(await rl(key, 2, 1000)).toBe(false)
  })
  ...
  it('allows requests again after the window expires', async () => {
    ...
    await rl(key, 2, windowMs)
    await rl(key, 2, windowMs)
    expect(await rl(key, 2, windowMs)).toBe(false)

    // Advance time past the window
    vi.advanceTimersByTime(windowMs + 1)

    expect(await rl(key, 2, windowMs)).toBe(true)
  })
  ...
  it('different keys are tracked independently', async () => {
    ...
    await rl(keyA, 1, 1000)
    expect(await rl(keyA, 1, 1000)).toBe(false)
    // keyB is unaffected
    expect(await rl(keyB, 1, 1000)).toBe(true)
  })

  it('limit of 1 allows exactly one request', async () => {
    const rl = await getRL()
    const key = `test-limit1-${Math.random()}`
    expect(await rl(key, 1, 1000)).toBe(true)
    expect(await rl(key, 1, 1000)).toBe(false)
  })
})
